...
function showOrder() {
    ...
    if (orders.length === 0) {
        orderHtml = `<tr><td colspan="6">Không có đơn hàng nào</td></tr>`;
    } else {
        orders.forEach((order, index) => {
            const status = order.trangthai === "Chưa xử lý" ? `<span class="status-no-complete">Chưa xử lý</span>` : `<span class="status-complete">Đã xử lý</span>`;
            const orderDate = new Date(order.thoigiandat).toLocaleDateString("vi-VN");
            ...
            orderHtml += `
                <tr>
                    ...
                    <td>${status}</td>
                    <td>
                        <button class="btn-detail" onclick="showOrderDetails('${order.id}')"><i class="fa-regular fa-eye"></i> Chi tiết</button>
                    </td>
                </tr>`;
        });
    }
...
// Hiển thị chi tiết đơn hàng khi bấm vào nút 'Chi tiết'
function showOrderDetails(orderId) {
    ...
    if (order && details) {
        let detailHtml = details.map(item => `
        ...
        document.getElementById("order-detail-content").innerHTML = `
            ...
            ${detailHtml}
            <p>Tổng tiền: ${vnd(order.tongtien)}</p>
        `;
    }
}


// Đặt trạng thái đơn hàng thành 'Đã xử lý'
function markOrderAsProcessed(orderId) {
    const orders = JSON.parse(localStorage.getItem("order")) || [];
    const orderIndex = orders.findIndex(order => order.id === orderId);

    if (orderIndex !== -1) {
        orders[orderIndex].trangthai = "Đã xử lý";
        localStorage.setItem("order", JSON.stringify(orders));
        showOrder(); // Cập nhật lại danh sách đơn hàng
    }
}

// Gọi showOrder để hiển thị danh sách đơn hàng khi tải trang admin
