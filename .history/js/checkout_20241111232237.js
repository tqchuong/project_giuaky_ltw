...
function handleOrderNow(productId) {
    addToCart(productId); // This function adds the product to the cart
    setTimeout(() => {
...
function showProductCart() {
    ...
    currentuser.cart.forEach(item => {
        let product = getProduct(item); // Hàm getProduct phải trả về thông tin chính xác của sản phẩm
        listOrderHtml += `<div class="food-total">
            <div class="count">${product.soluong}x</div>
            <div class="info-food">
                <div class="name-food">${product.title}</div>
                <div class="price">${vnd(product.price * product.soluong)}</div> <!-- Hiển thị giá từng sản phẩm -->
            </div>
        </div>`;
    });
