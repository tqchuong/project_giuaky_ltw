...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Kiểm tra sự tồn tại của signupButton trước khi gán sự kiện
    if (signupButton) {
        signupButton.addEventListener('click', (event) => {
            ...
            // Kiểm tra validate cho các trường nhập liệu
    ...
    // Kiểm tra sự tồn tại của loginButton trước khi gán sự kiện
    if (loginButton) {
...
function kiemtradangnhap() {
    ...
    if (currentUser) {
        ...
        // Tạo menu dựa trên loại tài khoản người dùng
        const userMenuElement = document.getElementById('user-menu');
        const menuHtml = generateMenuHtml(currentUser);
        userMenuElement.innerHTML = menuHtml;
        userMenuElement.style.display = 'none'; // Ẩn menu mặc định

        // Thêm sự kiện để mở menu khi di chuột vào `authContainer`
        authContainer.addEventListener('mouseover', (event) => {
            userMenuElement.style.display = 'block';
        });

        // Ẩn menu khi di chuột ra khỏi `authContainer`
        authContainer.addEventListener('mouseleave', (event) => {
            userMenuElement.style.display = 'none';
        });

        // Đăng sự kiện click cho các mục menu để không tự động chuyển vào "Tài khoản của tôi"
        userMenuElement.addEventListener('click', (event) => {
            event.stopPropagation(); // Ngăn chặn sự kiện nổi bọt
        });

        // Thêm sự kiện click cho các mục trong menu
        document.querySelectorAll('#user-menu a').forEach((item) => {
            item.addEventListener('click', (event) => {
                const targetId = event.target.getAttribute('href');
                if (targetId === "javascript:void(0);") {
                    showAccountInfo(); // Nếu mục là "Tài khoản của tôi"
                } else {
                    // Tương ứng với các mục khác như đơn hàng, quản lý cửa hàng, hoặc thoát tài khoản
...
// Xây dựng HTML menu dựa trên loại tài khoản của người dùng
function generateMenuHtml(user) {
    ...
    if (user.userType === 1) {
        // Nếu là admin, thêm mục quản lý cửa hàng
        menuHtml = `<li><a href="admin.html"><i class="fa-solid fa-gear"></i> Quản lý cửa hàng</a></li>` + menuHtml;
    }

    menuHtml += `<li class="border"><a href="javascript:;" onclick="logOut()"><i class="fa-solid fa-right-from-bracket"></i> Thoát tài khoản</a></li>`;
    return menuHtml;
}
