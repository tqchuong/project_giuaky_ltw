...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Xử lý sự kiện click vào auth-container (tên tài khoản)
    ...
    if (authContainer) {
        authContainer.addEventListener('click', (event) => {
            ...
            // Toggle hiển thị menu khi click vào tên tài khoản
            userMenuElement.style.display = userMenuElement.style.display === 'block' ? 'none' : 'block';
        });
    }

    // Đóng menu khi click ra ngoài vùng auth-container
...
// Ẩn tất cả các phần trang trừ phần cần hiển thị
function hideAllSections() {
    document.getElementById('trangchu').style.display = 'none';
    document.getElementById('account-user').style.display = 'none';
    document.getElementById('order-history').style.display = 'none';
...
// Kiểm tra đăng nhập và hiển thị thông tin dựa trên loại người dùng
function kiemtradangnhap() {
    ...
    if (currentUser) {
        ...
        // Tạo menu theo vai trò của người dùng
...
// Tạo nội dung menu dựa trên vai trò của người dùng
function generateMenuHtml(user) {
    ...
    // Thêm mục "Quản lý cửa hàng" cho admin
    if (user.userType === 1) {
        menuHtml = `<li><a href="admin.html"><i class="fa-solid fa-gear"></i> Quản lý cửa hàng</a></li>` + menuHtml;
    }

    menuHtml += `<li class="border"><a id="logout" href="javascript:void(0);" onclick="logOut()"><i class="fa-solid fa-right-from-bracket"></i> Thoát tài khoản</a></li>`;
    return menuHtml;
}
