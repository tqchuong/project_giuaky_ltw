...
function createAdminAccount() {
    ...
    if (!adminExists) {
        accounts.push({
            id: 1, // Thêm id cho tài khoản admin
...
function initializeReviews() {
    if (localStorage.getItem('reviews') == null) {
        ...
        // Giả sử admin là người dùng đầu tiên trong danh sách accounts
        let adminUser = accounts.find(account => account.userType === 1);
        if (!adminUser) {
            console.error("Admin account not found for reviews initialization.");
            return;
        }
...
// Gọi các hàm khởi tạo khi trang được tải
window.onload = function () {
    localStorage.clear(); // Xóa toàn bộ dữ liệu trước khi khởi tạo
    createAdminAccount();
    createProduct();
    initializeReviews();
};