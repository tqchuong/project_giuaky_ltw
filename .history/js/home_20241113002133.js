...
function adjustQuantity(e, isIncreasing) {
    const qtyInput = e.parentNode.querySelector('.input-qty');
    const currentQty = parseInt(qtyInput.value);
    const newQty = isIncreasing ? Math.min(currentQty + 1, qtyInput.max) : Math.max(currentQty - 1, qtyInput.min);
    qtyInput.value = newQty;
}

// Hiển thị carousel ảnh sản phẩm
function getProductCarousel(product) {
    const images = product.images?.length ? product.images : [product.img];
    return `
        <div class="product-carousel">
            <div class="carousel-images">
                ${images.map((imgSrc, i) => `<img src="${imgSrc}" class="carousel-image ${i === 0 ? 'active' : ''}">`).join('')}
            </div>
            <button class="carousel-prev" onclick="navigateCarousel(-1)">‹</button>
            <button class="carousel-next" onclick="navigateCarousel(1)">›</button>
        </div>
    `;
}

// Hiển thị phần đánh giá sản phẩm
function getReviewSection(index, reviews) {
    return `
        <div class="product-reviews">
            <h3>Đánh giá từ khách hàng:</h3>
            ${reviews.map(({ User_id, comment }) => `<div class="review"><strong>${User_id}</strong>: ${comment}</div>`).join('')}
            <h3>Thêm đánh giá của bạn:</h3>
            <div class="add-review">
                <label for="rating">Điểm đánh giá (1-5):</label>
                <input type="number" id="rating" min="1" max="5" value="5">
...
function getDetailSection(detail) {
    return `
        <h3>Chi tiết sản phẩm</h3>
        ${Object.entries(detail).map(([key, value]) => `<p><strong>${key}:</strong> ${value || 'N/A'}</p>`).join('')}
    `;
}

// Footer modal sản phẩm
function getFooterControls(product) {
    return `
        <span>Thành tiền: ${vnd(product.price)}</span>
        <button onclick="addToCart(${product.id})">Thêm vào giỏ hàng</button>
        <button onclick="handleOrderNow(${product.id})">Đặt hàng ngay</button>
    `;
}
// Đặt hàng ngay
...
function updateCartCount() {
    ...
    document.querySelector('.count-product-cart').innerText = cart.reduce((sum, item) => sum + item.quantity, 0);
}
