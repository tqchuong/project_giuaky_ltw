$(document).ready(function () {
    ...
    updateAmount(); // Update the cart count icon when the page loads
    ...
    function displayCart() {
        ...
        updateAmount(); // Update cart count icon
    }
    ...
    function updateEventListeners() {
        $(".btn-quantity.plus").off("click").on("click", function (e) {
            ...
            calculateTotal();
            updateLocalStorage();
            updateAmount(); // Update cart count icon after quantity change
        });
        ...
        $(".btn-remove").off("click").on("click", function () {
            ...
            updateLocalStorage();
            calculateTotal();
            updateAmount(); // Update cart count icon after item removal
        });
    }
    ...
    function updateAmount() {
        ...
        const totalItems = cart.reduce((total, item) => total + item.quantity, 0); // Sum of all item quantities
        $(".count-product-cart").text(totalItems); // Update the cart count icon
        localStorage.setItem('cartCount', totalItems); // Store count in localStorage for persistence
    }
