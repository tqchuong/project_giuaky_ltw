...
// Hàm để lấy số lượng sản phẩm từ giỏ hàng trong localStorage
function getAmountCart() {
    let cart = JSON.parse(localStorage.getItem('cart')) || [];
    return cart.reduce((sum, item) => sum + item.quantity, 0); // Tính tổng số lượng sản phẩm
}

// Hàm để cập nhật số lượng sản phẩm trên biểu tượng giỏ hàng
function updateAmount() {
    if (localStorage.getItem('currentuser') != null) { // Kiểm tra người dùng đăng nhập
        let amount = getAmountCart();
        document.querySelector('.count-product-cart').innerText = amount; // Cập nhật số lượng lên biểu tượng giỏ hàng
    }
}

// Gọi hàm updateAmount ngay khi trang được tải
document.addEventListener('DOMContentLoaded', function () {
    updateAmount(); // Cập nhật số lượng sản phẩm khi trang tải xong
});
...
// Ví dụ khi xóa sản phẩm khỏi giỏ hàng
